...
def Deeplabv3(weights='pascal_voc', input_tensor=None, input_shape=(512, 512, 3), classes=21, backbone='mobilenetv2'
    ...
    b4 = UpSampling2D((int(np.ceil(input_shape[0] / OS)), int(np.ceil(input_shape[1] / OS))), interpolation="bilinear")(b4)
    ...
    if backbone == 'xception':
        ...
        x = UpSampling2D(size=(int(np.ceil(input_shape[0] / 4)), int(np.ceil(input_shape[1] / 4))), interpolation="bilinear")(x)
